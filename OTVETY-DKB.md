...
| CSDINPROJT-25801 Pentest | API и UI **создаются/меняются** → pentest **нужен**, не CANCELED. | В BACKLOG нельзя: нет bitbucket, нет 2 УЗ на роль, нет контактов PO/Dev/QA TL. Черновик обязательного блока — ниже. |
...
В BACKLOG **нельзя** переводить: обязательные пункты 1–4 из инструкции закрыты не полностью (нет bitbucket, нет двух УЗ на каждую роль).
...
2. Контакты: Product Owner / Dev Teamlead / QA Teamlead / Аналитик — ДОПИСАТЬ ФИО и почту.
   Сейчас в задачах: аналитика Самсонова А., Забарянский Ю.Г., middle Гурин П.А. / Яковлев М.С.,
   фронт Никитенко А.А.; менеджмент Гапонова Е.Н. РП в эпике Inactive.
...
Статус BACKLOG не ставим, пока нет п.2 (контакты), п.3 (bitbucket) и двух УЗ на роль.
```
...
- Swagger на стенде.
- Product Owner, Dev TL, QA TL (ФИО + контакты).
...
3. **Контакты:** Product Owner, Dev Teamlead, QA Teamlead, аналитик (ФИО, почта, rocket).
...
Пока этого нет, в Jira можно **уже сегодня** положить черновики 25797 / 25798 / 25802 как позицию команды и **не** переводить 25801 в BACKLOG.
